...
class AtlasBrowser:  # noqa: D414
    ...
    def find_regions(
        self, coordinates: np.ndarray, plot: bool = False, performances=None
    ) -> list[str]:
        """Find the regions associated with MNI coordinates for the atlas.
        ...
        if plot and not self._plotting_ready:
            self._prepare_plotting(performances)
    ...
    def _prepare_plotting(self, performances) -> None:
        ...
        self._plotting_image = self._assign_colour_ids(plotting_image, performances)
        self._plotting_ready = True

    def _assign_colour_ids(self, image: np.ndarray, performances) -> np.ndarray:
        ...
        """
        colour_assignment = self._define_colour_assignment(performances)

        return self._assign_colours(image, colour_assignment)

    def _define_colour_assignment(self, performances=None) -> dict:
            ...
            for region_id, region_name in self._region_names.items():
                if region_name.startswith(unique_name):
                    if performances is not None:
                        # check if region_name is a region in the performances df
                        if region_name in performances["region"].values:
                            # get the performance of the region
                            perf = performances.query("region == @region_name")[
                                "performance_test"
                            ].values[0]
                            # if the performance is above 0.5, assign the colour
                            # else assign 0
                            if perf > 0.5:
                                colour_assignment[region_id] = perf
                            else:
                                colour_assignment[region_id] = 0.5
                        else:
                            colour_assignment[region_id] = 0.5

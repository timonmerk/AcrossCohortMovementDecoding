...
for cohort in ch_all.keys():
    for sub in ch_all[cohort].keys():
        ...
        if len(runs) > 1:
            dat_concat = np.concatenate([ch_all[cohort][sub][ch_best][run]["data"] for run in runs], axis=0)
            lab_concat = np.concatenate([ch_all[cohort][sub][ch_best][run]["label"] for run in runs], axis=0)
        else:
        ...
        # ideally check here also an embedding
